...
public class ImageIo {
    ...
    public static double[] getDoubleImageArray1DFromBufferedImage(BufferedImage image) {
        byte[] byteData_1d;
        double[] doubleData_1d;
        int Rows = image.getHeight(); int Cols = image.getWidth();
        int i, m,n,i4, blue, pixelLength;
        getBufferedImageType(image, null /*"getImageArray1DFromBufferedImage"*/);
        //System.out.println("Row_Siz= " + Rows + " Cols_Size = " + Cols);
        doubleData_1d = new double[image.getHeight() * image.getWidth()];
        byteData_1d = ((DataBufferByte) image.getRaster().getDataBuffer()).getData();
       // System.out.println("Image Length (1-D Array) = " + byteData_1d.length);
        ...
        for (i = 0; i < byteData_1d.length; i += pixelLength) {
            ...
            doubleData_1d[i4] = blue;                // choose the blue; intData_1d[i] intData_1d[i+1] intData_1d[i+2] intData_1d[i+3]; a,g,b,r;
        }
...
//        TYPE_CUSTOM
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                if ((int) (original[i][j] & 0xFF) >= th) {
                    original[i][j] = (byte) 255;//White
                } else {
                    original[i][j] = (byte) 0;//Black
                }
            }
        }
        return original;
    ...
    //Make the object white amd background black
    public static byte[][] thresholdReverse(byte[][] original, int th) {
        int h = original.length;
        int w = original[0].length;
